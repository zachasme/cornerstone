...
import csc from '../src/cornerstone.js';

test("min pixel < 0", function (t) {
    t.plan(4);
    ...
    // Assert
    t.equal(image.lut.length, 3, "lut length is 3");
    t.equal(image.lut[0], 0, "lut entry -1 is 0");
    t.equal(image.lut[1], 128, "lut entry 0 is 128");
    t.equal(image.lut[2], 255, "lut entry 1 is 255");
});
