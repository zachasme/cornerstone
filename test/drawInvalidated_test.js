...
describe('drawInvalidated', function () {
  ...
  it('should draw only the invalidated elements', function (done) {
    ...
    // Assert
    element1.addEventListener('imagerender', function () {
      // If element1 is redrawn, then this test has failed.
      // Only element2 should be redrawn.
      done();
    });

    element2.addEventListener('imagerender', function ({ detail }) {
      // Make sure element2 is redrawn since it has been invalidated
      assert.equal(detail.element, element2);
      assert.equal(detail.image, image2);
      done();
    });
  ...
  it('should draw all invalidated elements', function (done) {
    ...
    // Assert
    element1.addEventListener('imagerender', function ({ detail }) {
      // If element1 is redrawn, then this test has failed.
      // Only element2 should be redrawn.
      assert.equal(detail.element, element1);
      assert.equal(detail.image, image1);
      done();
    });
